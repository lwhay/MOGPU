...
__global__ void UpdateKernel(GConfig * dev_p_gconfig) {
    ...
    while (true) {
        if (tid == 0) {
            ...
            if (rebalance == 1 ||
                (buffer_exhausted == 1 && req_cache_update->token0 == 0 && req_cache_update->token1 == 0)) {
                ...
                //                != p_cell_local->tot_obs)
                    ...
                    if (p_cell_local->tot_obs_top <= 0 && p_cell_local->cnt_bkts > 1) {
                        if (tid_w == 0) {
                            ...
                            queue_bkts_free->avail_idx_bkt[idx_in_queue %
                                                           queue_bkts_free->capacity] = p_grid_local->getArrIdxBkt(
                                    p_cell_local->idx)[cnt_bkts];
                ...
                while (anchor < tmp_cnt) {
                    ...
                    if (p_cell_local->tot_obs_top > len_bkt - realWrapSize) {
                        ...
                        if (tid_w == 0) {
                            ...
                            int idx_bkt_in_pool = queue_bkts_free->avail_idx_bkt[idx_in_queue %
